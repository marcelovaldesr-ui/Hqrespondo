...
/** GET /api/growth/ideas — ideas creadas por el equipo (solo BD). */
...
/** POST /api/growth/ideas — crea una idea. responsable sale de x-hq-user. */
export async function POST(req: Request) {
  try {
    ...
    for (const c of CAMPOS) {
      if (c in body && body[c] !== undefined) insert[c] = body[c];
    }
    ...
    if (error) {
      if (tablaFaltante(error.message))
        return NextResponse.json(
          {
            error:
              "Falta ejecutar la migración 009_growth_studio.sql en Supabase para guardar ideas. Mientras tanto puedes usar el seed y copiar el contenido.",
          },
          { status: 409 },
        );
      throw new Error(error.message);
